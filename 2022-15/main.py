...
def solve1(file, y_index):
    ...
    for i, line in enumerate(lines):
        ...
        parts_with_numbers_cleaned = [word.replace(',', '').replace(
            ':', '') for word in parts_with_numbers]
        sensor_x, sensor_y, beacon_x, beacon_y = list(
            map(lambda x: int(x.split('=')[1]), parts_with_numbers_cleaned))
        ...
        if y_index in range(sensor_y - distance_to_beacon, sensor_y + distance_to_beacon):
            save_no_beacon_zone(line_overlaps, sensor_x,
                                sensor_y, distance_to_beacon, y_index)
